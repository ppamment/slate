 ...
 * ========================= */
	...
	Lightbox.prototype.preloadSize = function(callback)
	{
        ...
        var caption = that.$element.find(".lightbox-caption");
        if(caption.length > 0){
            that.$element.css({visibility: "hidden", display: "block", position: "absolute"});
            captionHeight = parseInt( caption.outerHeight() );
            that.$element.css({visibility: "visible", display: "none", position: "static"});
        } else {
            captionHeight = 0;
        }
		...
		preloader.onload = function()
		{
			...
			if( (preloader.height + padTop + padBottom + captionHeight) >= windowHeight)
			{
				originalWidth = preloader.width;
				originalHeight = preloader.height;
				preloader.height = windowHeight - padTop - padBottom - captionHeight;
			...
			that.$element.css({
				...
				'height': preloader.height + padTop + padBottom + captionHeight,
				'top' : (windowHeight / 2) - ( (preloader.height + padTop + padBottom + captionHeight) / 2),
				'left' : '50%',
				'margin-left' : -1 * (preloader.width + padLeft + padRight) / 2
			});
			that.$element.find('.lightbox-content').css({
				'width': preloader.width,
				'height': preloader.height + captionHeight
			});
